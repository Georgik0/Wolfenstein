...
int		get_img_1(t_vars *vars)
{
	...
	if (!(vars->data_array[1]->img = mlx_xpm_file_to_image(vars->mlx,
	...
	&(vars->data_array[2]->line_length), &(vars->data_array[2]->endian));
	return (1);
}

int		get_img_2(t_vars *vars)
{
	int		img_width;
	int		img_heigh;
	...
	&(vars->data_array[4]->line_length), &(vars->data_array[4]->endian));
	if (!(vars->data_array[5]->img = mlx_xpm_file_to_image(vars->mlx,
	...
	&(vars->data_array[5]->line_length), &(vars->data_array[5]->endian));
	return (1);
}
...
int		get_img(t_vars *vars)
{
	...
	&(vars->data_array[0]->line_length), &(vars->data_array[0]->endian));
